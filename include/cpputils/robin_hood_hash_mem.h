#ifndef __CPPUTILS_ROBIN_HOOD_HASH_MEM_H__
#define __CPPUTILS_ROBIN_HOOD_HASH_MEM_H__
...
public:
    RobinHoodHashMem() : m_table(nullptr) {}
    ...
    const Value* Lookup(const Key& key) const {
        return const_cast<RobinHoodHashMem*>(this)->Lookup(key);
    }
    ...
    void ForEach(const std::function<bool (uint32_t slot, uint32_t psl, const Value& v)>& f) const {
        const_cast<RobinHoodHashMem*>(this)->ForEach([&f] (uint32_t slot, uint32_t psl, Value* v) -> bool {
            return f(slot, psl, *v);
        });
    }
...
public:
    RobinHoodHashMem(RobinHoodHashMem&&) = default;
    RobinHoodHashMem& operator=(RobinHoodHashMem&&) = default;

private:
    RobinHoodHashMem(const RobinHoodHashMem&) = delete;
    RobinHoodHashMem& operator=(const RobinHoodHashMem&) = delete;
};
...
template <typename Key, typename Value,
          typename KeyHash = std::hash<Key>,
          typename KeyEqual = std::equal_to<Key>>
using RobinHoodHashMap = RobinHoodHashMem<Key, std::pair<Key, Value>,
...
template <typename Key,
          typename KeyHash = std::hash<Key>,
          typename KeyEqual = std::equal_to<Key>>
using RobinHoodHashSet = RobinHoodHashMem<Key, Key,
